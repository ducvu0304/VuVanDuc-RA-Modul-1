...
function getRenderData() {
    document.querySelector('.p_count').innerHTML = `(${productData.length})`;

    let filterDataPrice =  filterDataByPrice(productData, sliderValueMin.value, sliderValueMax.value);
    let filterDataCategory = filterByCategory(filterDataPrice,  filter.checkNewArrival, filter.checkSaleProduct)
    
    renderData = filterDataCategory;
...
function getPriceElement(id) {
    let element = '';
 
    for (let i in renderData) {
        if(renderData[i].id == id) {
            if(renderData[i].other.match("SALE")) {
                let newPrice = Math.ceil(renderData[i].price*(1 - (renderData[i].discount/100)));
                element = `<li class="product-price"><span class="old-price">${changeVND(renderData[i].price.toString())} ₫</span><span class="new-price">${changeVND(newPrice.toString())} ₫</span></li>`
            }else {
                element = `<li class="product-price">${changeVND(renderData[i].price.toString())} ₫</li>`
            }
        }
    }
            ...
            ` <div onclick="getProduct(${renderData[i].id})">
                <div class="bucket">
                    <ul class="product">
                        ...
                        ${getPriceElement(renderData[i].id)}
                    </ul>
                </div>                    
            </div>`     
        }else{
